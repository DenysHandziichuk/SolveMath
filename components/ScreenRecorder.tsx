...
export function ScreenRecorder() {
  ...
  const startRecording = async () => {
    ...
    try {
      // 1. Capture the current tab/window
      ...
      // Use high DPI scaling for crisp math text
      ...
      // This tells us exactly where the webpage content starts relative to the window frame
      const getOffsets = () => {
        ...
        // Fallback for other browsers (standard inner/outer difference)
        return {
          x: (window.outerWidth - window.innerWidth) / 2,
          y: window.outerHeight - window.innerHeight // Assumes navbar is at top
        };
      };
      ...
      const drawFrame = () => {
        if (ctx && hiddenVideo.readyState >= 2) {
          ...
          // Source coordinates need to account for the browser's navbar/chrome
          // We map the DOM position to the Video Stream position
          ctx.drawImage(
            hiddenVideo,
      ...
      // 3. Get the stream from our "Virtual Canvas"
      ...
      const mediaRecorder = new MediaRecorder(croppedStream, mimeType ? { mimeType } : undefined);
